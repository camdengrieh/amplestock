...
contract MockFeedRegistry is IFeedRegistry {
    ...
    /// @notice The gate this registry reads the session from. Purely informational here.
    ...
    function maxAge(address token, Session session) external view returns (uint32 bound) {
        ...
        bound = uint32(uint256(_configs[token].heartbeat) * _freshnessMultiplier[uint8(session)] / 100);
    }

    /// @inheritdoc IFeedRegistry
    function freshnessMultiplier(Session session) external view returns (uint16 multiplier) {
        multiplier = _freshnessMultiplier[uint8(session)];
    }
    ...
    function setFreshnessMultiplier(Session session, uint16 multiplier) external {
        _freshnessMultiplier[uint8(session)] = multiplier;
