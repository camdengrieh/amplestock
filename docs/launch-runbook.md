...
| 13 | **The migration predicate measured against the real Stock Tokens.** For every constituent, record what `IStockToken.isBlocked(vault)` answers and whether a **zero-value self-transfer** succeeds inside `Constants.STOCK_TOKEN_PROBE_GAS` (50,000), with the vault's habitual zero balance. `VaultNavLib.migrationPredicate` unlocks `emergencyMigrate` on one `isBlocked` **or** two failed probes, and the probe moves 1 wei only when the vault holds at least 1 wei — so two tokens that reject zero-value or self transfers, or that exceed the gas cap, would satisfy the predicate permanently and the guardian's evacuation would be unlocked for a reason that is not an incident. It is a fact about the deployed tokens rather than about the protocol, so it is measured rather than assumed (audit wave 5, dispositioned lead). If two or more constituents fail the probe in the healthy state, say so in the launch note: the standby is still gated by its own registration and 14-day tier, and no migration can happen without the guardian Safe choosing to make one | probe report, per constituent, on 46630 and on the launch chain |
